...
#args.foldernamesig, INPUT_PATH_SIG)
# problem definition
...
if NS:
    ...
    folder_out += '/'
NP = 'v2-MUMU%iD_reffixed_NR%i_NB%i_NS%i_M%i_lam%s_naggr%i'%(len(features_tr), NR, NB, NS, M, str(lam), n_aggreg)
...
for i in range(Ntoys):
    ...
    t_sum, t_aggrD, t_aggrR = run_aggr2(i, n_aggreg, NP, feature_all_tr, target_all,  weight=weight_Ref,
                                        ...
                                        xlabels_list=features_read, yrange_dict=yrange_dict, features_plot=feature_all,
    )
