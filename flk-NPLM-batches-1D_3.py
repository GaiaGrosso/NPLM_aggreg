...
if NS:    
    NP = 'v3-1D-EXPO_NR%i_NB%i_NS%i_Sloc%s_Sstd%s_M%i_lam%s_naggr%i'%(NR, NB, NS, str(Sig_loc), str(Sig_std), M, str(lam), n_aggreg)
else:
    NP = 'v3-1D-EXPO_NR%i_NB%i_NS%i_M%i_lam%s_naggr%i'%(NR, NB, NS, M, str(lam), n_aggreg)
