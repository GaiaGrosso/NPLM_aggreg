...
if NS:    
    NP = 'v5-1D-EXPO_NR%i_NB%i_NS%i_Sloc%s_Sstd%s_M%i_lam%s_naggr%i'%(NR, NB, NS, str(Sig_loc), str(Sig_std), M, str(lam), n_aggreg)
else:
    NP = 'v5-1D-EXPO_NR%i_NB%i_NS%i_M%i_lam%s_naggr%i'%(NR, NB, NS, M, str(lam), n_aggreg)
...
for i in range(Ntoys):
    ...
    t_sum, t_aggrD, t_aggrR = run_aggr5(i, n_aggreg, NP, feature_all, target_all,  weight=weight_Ref,
...
#f.create_dataset('seed_flk', data=np.append(seeds_flk_past_dict, seeds_flk_dict), compression='gzip')
