...
if config_json["N_Sig"]:
    if config_json["is_NP2"]:
        ID = 'NPLMaggreg_%iD_%sNR%i_NB%i_NS%i_tail-excess'%(1+config_json["N_flat_extra"], is_pois, config_json['N_Ref'], config_json['N_Bkg'], config_json['N_Sig'])
    else:
        ID = 'NPLMaggreg_%iD_%sNR%i_NB%i_NS%i_loc%s_std%s'%(1+config_json["N_flat_extra"], is_pois, config_json['N_Ref'], config_json['N_Bkg'], config_json['N_Sig'],
                                           str(config_json['Sig_loc']), str(config_json['Sig_std']))
else:
    ID = 'NPLMaggreg_%iD_%sNR%i_NB%i_NS%i'%(1+config_json["N_flat_extra"], is_pois, config_json['N_Ref'], config_json['N_Bkg'], config_json['N_Sig'])
...
if __name__ == '__main__':
    ...
    if args.local:
        ...
        for i in range(njobs):
            ...
            script_sbatch.write("#SBATCH -p iaifi_gpu\n")
