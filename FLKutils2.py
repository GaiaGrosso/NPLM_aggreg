...
def run_toy(test_label, X_train, Y_train, weight, flk_config, seed, plot=False, verbose=False, savefig=False, output_path='', df=10, features_plot=[]):
    '''
    ...
    with open(output_path+"t.txt", 'a') as f:
        f.write('{},{}\n'.format(seed,t))
    if plot:
        if not len(features_plot):
                features_plot=X_train
        plot_reconstruction(data=features_plot[Y_train.flatten()==1], weight_data=1, ref=features_plot[Y_train.flatten()==0], weight_ref=weight,
...
def run_aggr(n_aggreg, test_label, X_train, Y_train, weight, flk_config, seed, xlabels_list=[], yrange_dict=None, plot=False, verbose=False, savefig=False, output_path='', features_plot=[]):
    '''                                                                                                                                                                                      
    type of signal: "NP0", "NP1", "NP2", "NP3"                                                                                                                                               
    output_path: directory (inside ./runs/) where to save results                                                                                                                       
    N_0: size of ref sample                                                                                                                                       
    ...
    if plot:
        for m in range(n_aggreg):
            if not len(features_plot):
                features_plot=X_train
            plot_reconstruction_ensemble(data=features_plot[m][Y_train[m].flatten()==1], weight_data=1, ref=features_plot[m][Y_train[m].flatten()==0], weight_ref=weight,
                                         ref_preds=[preds_dict[str(n)][str(m)][Y_train[m].flatten()==0] for n in range(n_aggreg)], ref_preds_aggreg=FR_dict[str(m)],
                                         save=savefig, save_path=output_path+'/plots/', file_name=test_label+'seed%i_aggr%i'%(seed, m),
...
def run_aggr3(toy, n_aggreg, test_label, X_train, Y_train,  weight, flk_seeds, flk_sigmas, flk_M, flk_lam, flk_dfs,
              xlabels_list=[], yrange_dict=None, plot=False, verbose=False, savefig=False, output_path='', features_plot=[]):
    preds_dict = {}
    for n in range(n_aggreg):
        preds_dict[str(n)] = {}
        pval_tmp = np.array([])
    ...
    if plot:
        for m in range(n_aggreg):
            ...
            plot_reconstruction_ensemble(data=features_plot[m][Y_train[m].flatten()==1], weight_data=1, ref=features_plot[m][Y_train[m].flatten()==0], weight_ref=weight,
                                         ref_preds=[preds_dict[str(n)][str(m)][Y_train[m].flatten()==0] for n in range(n_aggreg)], ref_preds_aggreg=FR_dict[str(m)],
                                         save=savefig, save_path=output_path+'/plots/', file_name=test_label+'toy%i-aggr%i'%(toy, m),
                                         xlabels=xlabels_list, yrange=yrange_dict,
            )
    return t_sum, t_aggreg_D, t_aggreg_R
...
def plot_reconstruction_ensemble(data, weight_data, ref, weight_ref, ref_preds_aggreg, ref_preds=[],t_obs=None, df=None, title='', xlabels=[], yrange=None, binsrange=None, save=False, save_path='', file_name=''):
    '''                                                                                                                                                                                                                      
    Reconstruction of the data distribution learnt by the model.                                                                                                                                                                                                                                                                                                                                                                     
    df:              (int) chi2 degrees of freedom                                                                                                                                                                            
    data:            (numpy array, shape (None, n_dimensions)) data training sample (label=1)                                                                                                                                 
    ...
    for i in range(data.shape[1]):
        ...
        if save:
            ...
            plt.savefig(save_path+file_name+'_%i.pdf'%(i))
        #plt.show()
        plt.close()
    return
